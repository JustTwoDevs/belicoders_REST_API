...
export const evaluateSolution = async (req, res, next) => {
  try {
    const { databaseName, inputUser, solution } = req.body;

    const useDbQuery = `USE ${databaseName}`;
    await executeSqlQuery(useDbQuery);
    const results = await executeSqlQuery(solution);
    const resultsUser = await executeSqlQuery(inputUser);
    const result = JSON.stringify(results);
    const resultUser = JSON.stringify(resultsUser);
